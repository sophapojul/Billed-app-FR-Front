...
describe('Given I am connected as an Admin', () => {
  ...
  describe('When I am on Dashboard page and I click on arrow', () => {
    test('Then, tickets list should be unfolding, and cards should appear', async () => {
      ...
      await screen.findByTestId('open-bill47qAXb6fIm2zOKkLzMro');
      expect(screen.getByTestId('open-bill47qAXb6fIm2zOKkLzMro')).toBeInTheDocument();
      icon2.addEventListener('click', handleShowTickets2);
      userEvent.click(icon2);
      expect(handleShowTickets2).toHaveBeenCalled();
      await screen.findByTestId('open-billUIUZtnPQvnbFnB0ozvJh');
      expect(screen.getByTestId('open-billUIUZtnPQvnbFnB0ozvJh')).toBeInTheDocument();

      icon3.addEventListener('click', handleShowTickets3);
      userEvent.click(icon3);
      expect(handleShowTickets3).toHaveBeenCalled();
      await screen.findByTestId('open-billBeKy5Mo4jkmdfPGYpTxZ');
      expect(screen.getByTestId('open-billBeKy5Mo4jkmdfPGYpTxZ')).toBeInTheDocument();
    });
  });
  ...
  describe('When I am on Dashboard page and I click 2 times on edit icon of a card', () => {
    test('Then, big bill Icon should Appear', () => {
      ...
      expect(bigBilledIcon).toBeInTheDocument();
    });
  });
  ...
  describe('When I am on Dashboard and there are no bills', () => {
    test('Then, no cards should be shown', () => {
      ...
      expect(iconEdit).not.toBeInTheDocument();
    });
  });
});
...
describe('Given I am a user connected as Admin', () => {
  describe('When I navigate to Dashboard', () => {
    test('fetches bills from mock API GET', async () => {
      ...
      await screen.findByText('Validations');
      const contentPending = await screen.getByText('En attente (1)');
      expect(contentPending).toBeInTheDocument();
      const contentRefused = await screen.getByText('Refusé (2)');
      expect(contentRefused).toBeInTheDocument();
      expect(screen.getByTestId('big-billed-icon')).toBeInTheDocument();
    });
